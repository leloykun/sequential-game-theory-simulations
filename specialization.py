...
class Cell(cellular.Cell):
    resources = [settings.MAX_RES_AMOUNT for _ in range(settings.RESOURCE_COUNT)]
    wall = False
    ...
    def colour(self):
        if self.wall:
            ...
            ratio = sum(self.resources) / (settings.MAX_RES_AMOUNT * settings.RESOURCE_COUNT)
    ...
    def load(self, data):
        if data == 'X':
            ...
            self.wall = False
            self.resources = [settings.MAX_RES_AMOUNT for _ in range(settings.RESOURCE_COUNT)]
            self.growthRate = settings.DEF_GROWTH_RATE

    def update(self):
        if settings.TO_UPDATE_CELLS:
            for i in range(settings.RESOURCE_COUNT):
                self.resources[i] += settings.MAX_RES_AMOUNT * self.growthRate[i]
                # normalize
                self.resources[i] = min(self.resources[i], settings.MAX_RES_AMOUNT)
