...
def worker(params):
    alpha, gamma, training_trials, test_trials, step, depth_a, depth_b, reward_per_cat = params
    ...
    while env.world.fed < training_trials:
        ...
        if (env.world.fed + 1) % step == 0:
            counter += 1
            print(output_dir + '/fed%d/%d.png' % (env.world.fed, counter))
            env.world.display.save_image(output_dir + '/fed%d/%d.png' % (env.world.fed+1, counter))
        else:
            counter = 0
...
def run(params, grid_params=False, test=False, to_save=True):
    training_trials, test_trials, step = process(params)
